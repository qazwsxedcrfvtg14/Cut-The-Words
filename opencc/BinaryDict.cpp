...
size_t BinaryDict::KeyMaxLength() const {
  ...
  for (const DictEntry* entry : *lexicon) {
    maxLength = max(maxLength, entry->KeyLength());
  }
  return maxLength;
}
