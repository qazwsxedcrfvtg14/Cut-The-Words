...
private:
  ...
  void ClearDict() {
    std::map<UTF8StringSlice8Bit, PhraseExtract::Signals>().swap(dict);
  }
  ...
  std::map<UTF8StringSlice8Bit, PhraseExtract::Signals> dict;
...
typedef std::map<PhraseExtract::UTF8StringSlice8Bit, size_t>
    AdjacentSetType;
...
double PhraseExtract::CalculateEntropy(const std::map<
    UTF8StringSlice8Bit, size_t>& choices) const {
